import requests
import hashlib
import time
import json
import base64
import os
import cv2
from flask import Flask, render_template,request, Response
from flask import redirect, url_for
from flask import jsonify 
import datetime
from twilio.rest import Client
temp_URL = None
image_path = None 
output_directory = "C:/Users/hande/OneDrive/Desktop/FaseApi/venv/Lib/site-packages/tempimage"
local_file_path = "C:/Users/hande/OneDrive/Desktop/FaseApi/venv/Lib/site-packages/tempvedio/downloaded_video.mp4"
#account_sid = 'AC23d6354e5b0419c47b32ba63292e6025'
#auth_token = 'fbbc789a344ba59710421d1f2ad8da88'
account_sid = 'AC5bb92d506f5cf06c89be032f17fc37e8'
auth_token = '7d6629901de0baa88ceb90d659703e1c'
client = Client(account_sid, auth_token)
api_key = "ZI6JGNIWI3IARPNWIADJBMUVPJKAIJAPBYH9ZHW8Q0U7QX4I7QK2IFJIFTJATEO58K9ZX7F9OM8MNHPNFBSW07AFZ5SOT1GP7SZAPDSVBFDNGIJ2SGUQ99CRUQ5IBBVE"
length = 8
content = "ALL"
endpoint_id = 122381
summary = None
# Usage:
base_url1 = "https://api.smarterai.com/v4/endpoints/122381/recordings"
headers = {
    "accept": "application/json",
    "SAI-Key": "ZI6JGNIWI3IARPNWIADJBMUVPJKAIJAPBYH9ZHW8Q0U7QX4I7QK2IFJIFTJATEO58K9ZX7F9OM8MNHPNFBSW07AFZ5SOT1GP7SZAPDSVBFDNGIJ2SGUQ99CRUQ5IBBVE"
}
current_timestamp = int(datetime.datetime.now().timestamp() * 1000)
end_timestamp = current_timestamp 
start_timestamp = current_timestamp - (5 * 60 * 1000)
start_time = current_timestamp - (5 * 60 * 1000)
#print("@@@@@@@@@@@@@@@@@@@@@@@@@",start_timestamp,"############",end_timestamp)
#start_timestamp = 1693458281367
#end_timestamp = 1693458286296
app = Flask(__name__)

def send_recording_request(api_key, length, content, endpoint_id,summary,start_time):
    url = "https://api.smarterai.com/v4/recording-requests"
    headers = {
        "SAI-Key": api_key,
        "accept": "application/json",
        "content-type": "application/json"
    }
    data = {
        "length": length,
        "content": content,
        "endpointId": endpoint_id,
        "summary":summary,
        "start_time":start_time
    }

    response = requests.post(url, headers=headers, json=data)
    response_data = response.json()
    
    requested_id = "" 
    
    if 'requestId' in response_data:
        requested_id = response_data['requestId']
    
    return response_data, requested_id  

response_data, requested_id = send_recording_request(api_key, length, content, endpoint_id,summary,start_time)
print("Response Data:", response_data)
print("Requested ID:", requested_id)
delay_seconds = 6
time.sleep(delay_seconds)

def get_recording_request(api_key, recording_request_id):
    base_url = f"https://api.smarterai.com/v4/recording-requests/{recording_request_id}"
    headers = {
        "accept": "application/json",
        "SAI-Key": api_key
    }
    
    try:
        #response = requests.post(base_url, headers=headers, json=data)
        response = requests.get(base_url, headers=headers)
        #response = requests.patch(base_url, headers=headers, json=data)
        if response.status_code == 200:
            recording_request_data = response.json()
            request_id = recording_request_data.get("requestId")
            endpoint_id = recording_request_data.get("endpointId")
            status = recording_request_data.get("status")
            summary = recording_request_data.get("summary")
            start_time = recording_request_data.get("startTime")
            length = recording_request_data.get("length")
            content = recording_request_data.get("content")
            return {
                "Request ID": request_id,
                "Endpoint ID": endpoint_id,
                "Status": status,
                "Summary": summary,
                "Start Time": start_time,
                "Length": length,
                "Content": content
            }
        else:
            return {"error": f"Request failed with status code: {response.status_code}"}
    except requests.exceptions.RequestException as e:
        return {"error": f"An error occurred: {e}"}

recording_request_id = requested_id  
#recording_request_id = "9334159c-b920-4b30-811c-547c3e200df7"
result = get_recording_request(api_key, recording_request_id)
if "error" in result:
    print(result["error"])
else:
    for key, value in result.items():
        print(f"{key}: {value}")
def get_recordings(base_url1, headers, start_timestamp, end_timestamp):
    global temp_URL
    params = {
        "from": start_timestamp,
        "to": end_timestamp,
        "client": 0
    }

    try:
        response = requests.get(base_url1, headers=headers, params=params)
        if response.status_code == 200:
            recordings_data = response.json()
            recordings = recordings_data.get("recordings", [])
            filtered_recordings = [recording for recording in recordings if recording["source"] == "vid_2" and recording["type"] == "VIDEO"]

            if filtered_recordings:
                first_matching_recording = filtered_recordings[0]
                temp_URL = first_matching_recording["url"]
                print("Recording ID:", first_matching_recording["id"])
                print("Start Timestamp:", first_matching_recording["startTimestamp"])
                print("End Timestamp:", first_matching_recording["endTimestamp"])
                print("Source:", first_matching_recording["source"])
                print("Type:", first_matching_recording["type"])
                print("URL:", temp_URL)
                print("-" * 40)
            else:
                print("No recordings matching the criteria found.")

        else:
            print("Request failed with status code:", response.status_code)
    except requests.exceptions.RequestException as e:
        print("An error occurred:", e)
    return temp_URL
if "error" in result:
    time.sleep(delay_seconds)
    get_recordings(base_url1, headers, start_timestamp,end_timestamp)
else:
    print(f"Status: {result['Status']}")
    status1 = {result['Status']}
    print(status1)
    print(f"Start Time: {result['Start Time']}")
    #start_timestamp = {result['Start Time']}
    #start_timestamp_str = result['Start Time']
    #start_timestamp = int(start_timestamp_str)
    #end_timestamp = start_timestamp - (10 * 60)
    #print("Start Timestamp (Unix):", start_timestamp)
    #print("End Timestamp (Unix):", end_timestamp)
    time.sleep(delay_seconds)
    get_recordings(base_url1, headers, start_timestamp,end_timestamp)
print("################################",temp_URL)
if temp_URL is None:
    time.sleep(30)
    get_recordings(base_url1, headers, start_timestamp,end_timestamp)
@app.route('/', methods=['GET', 'POST'])
def index():
    if temp_URL is None:
        return render_template('index2.html')
    else:
        return render_template('index1.html')
@app.route('/check_status', methods=['POST'])
def check_device_status():
    global device_status
    response_data, requested_id = send_recording_request(api_key, length, content, endpoint_id,summary,start_time)
    
    if requested_id is None:
        device_status = "Offline"
        '''message = client.messages.create(
        #from_='+12563048332',
        from_='+17157695307',
        body='Devise is offline',
        to='+919822442523'
        #to='+917775058954'
        )'''
    else:
        device_status = "Online"
        print(requested_id)
    
    return render_template('index1.html', device_status=device_status)



base_url = "https://frec-v2.edgetensor.com"  
api_key = "7fea772b-d454-49ab-a40d-338318c9ed3f"
secret_key = "1PY9UWDu1giCPEGDZkB6guVhYdybnx2SMK7xzLVJpnDjBMKtuFe52Q8n2T6N6CKO"
version = "v2"  

def generate_sha512_hash(data):
    return hashlib.sha512(data.encode("utf-8")).hexdigest()

def get_headers():
    timestamp = str(int(time.time() * 1000))
    sha512_hash = generate_sha512_hash(api_key + secret_key + timestamp)
    
    headers = {
        "X-API-KEY": api_key,
        "X-TIMESTAMP": timestamp,
        "X-SHA512-HASH": sha512_hash
    }
    return headers
'''def create_faceset(data):
    endpoint = f"{base_url}/{version}/facesets"
    headers = get_headers()
    
    response = requests.post(endpoint, json=data, headers=headers)
    return response.json()
faceset_data = {
    "project_id": "3581d0ca-2ad2-423d-980e-a5dc681faaea",
    "name": "Faceset-1"
}
created_faceset = create_faceset(faceset_data)
print("Created Faceset:", created_faceset)'''

def get_faces(faceset_id, limit, offset):
    endpoint = f"{base_url}/{version}/faces"
    headers = get_headers()

    params = {
        "faceset_id": faceset_id,
        "limit": limit,
        #"face_token":face_token,
        "offset": offset
    }

    try:
        response = requests.get(endpoint, headers=headers, params=params)

        if response.status_code == 200:
            faces_data = response.json()
            return faces_data
        else:
            return {"error": "Failed to retrieve faces", "status_code": response.status_code}

    except requests.exceptions.RequestException as e:
        return {"error": f"An error occurred: {e}"}

faceset_id_param = '81f151ad-cb9b-47a1-be10-3d3b2cb03120' 
limit_param = 10 
#face_token_param = "7a6260bb-76c3-477f-bedb-46673a49eeb6"
offset_param = 0  

faces_data = get_faces(faceset_id=faceset_id_param, limit=limit_param, offset=offset_param)
#print("Faces Data:", faces_data)

#compare face
def perform_face_lookup(lookup_data):
    endpoint = f"{base_url}/{version}/lookups/face"
    headers = get_headers()
    try:
        response = requests.post(endpoint, json=lookup_data, headers=headers)
        return response.json()
    except requests.exceptions.RequestException as e:
        return {"error": f"An error occurred: {e}"}

def perform_face_landmark_lookup(lookup_data):
    endpoint = f"{base_url}/{version}/lookups/landmark"
    headers = get_headers()
    try:
        response = requests.post(endpoint, json=lookup_data, headers=headers)
        return response.json()
    except requests.exceptions.RequestException as e:
        return {"error": f"An error occurred: {e}"}
if __name__ == "__main__":
    if temp_URL is None:
        print("Video URL is not available. Video download and processing skipped.")
        app.run()
    else:
        video_response = requests.get(temp_URL)
        video_data = video_response.content
        if video_response.status_code == 200:
            video_data = video_response.content
            with open(local_file_path, "wb") as video_file:
                video_file.write(video_data)
                print(f"Video downloaded and saved as {local_file_path}")
        else:
            print(f"Failed to download video. Status code: {video_response.status_code}")
    
        frame_count = 0
        cap = cv2.VideoCapture(local_file_path)
        while True:
            ret, frame = cap.read()
            if not ret:
                break  
            frame_count += 1
            if frame_count == 3: 
                image_filename = os.path.join(output_directory, "first_frame.jpg")
                cv2.imwrite(image_filename, frame)
                print(f"Saved the third frame as {image_filename}")
                break
        cap.release()
        cv2.destroyAllWindows()
        image_filename = os.path.join(output_directory, "first_frame.jpg")
        image = cv2.imread(image_filename)
        left = 100   
        top = 150    
        right = 300  
        bottom = 400 
        cropped_image = image[top:bottom, left:right]
        output_filename = os.path.join(output_directory, "cropped_image.jpg")
        cv2.imwrite(output_filename, cropped_image)  
        with open(image_filename, "rb") as image_file:
            image_data = base64.b64encode(image_file.read()).decode("utf-8")

        lookup_request_data = {
            "faceset_id": '81f151ad-cb9b-47a1-be10-3d3b2cb03120',
            "project_id": "3581d0ca-2ad2-423d-980e-a5dc681faaea",
            "size": 0,
            "merge_images_for_lookup": False,
            "lookup_request_landmarks": [
                {
                    "face_landmark": [
                        [0]
                    ],
                    "sequence_number": 0,
                    "base64string":  image_data,
                    "embedding_vector": [
                        0
                    ]
                }
            ],
            "lookup_request_faces": [
                {
                "sequence_number": 0,
                "base64string":image_data,
            }
            ]
        }
@app.route('/display_faces',methods=['GET', 'POST'])
def display_faces():
    faceset_id_param = '81f151ad-cb9b-47a1-be10-3d3b2cb03120'
    limit_param = 10
    offset_param = 0

    faces_data = get_faces(faceset_id=faceset_id_param, limit=limit_param, offset=offset_param)
    print(faces_data)
    return render_template('index1.html', faces=faces_data)
@app.route("/upload", methods=['POST'])
def upload():
    try:
        image_file = request.files.get("file")

        if image_file:
            image_path = "C:/Users/hande/OneDrive/Desktop/APIFACE23/selectedimage/" + image_file.filename
            print("image_path =",image_path)
            image_file.save(image_path)
            name = request.form.get("name")
            create_face(image_path, name)
            return render_template('index1.html')
        else:
            return "No file uploaded."
    except Exception as e:
        return f"An error occurred: {str(e)}"

print("image path =",image_path)
def create_face(image_path, name):
    faceset_id = '81f151ad-cb9b-47a1-be10-3d3b2cb03120'
    #name = "Akash"
    project_id = "3581d0ca-2ad2-423d-980e-a5dc681faaea"

    endpoint = f"{base_url}/{version}/faces"
    headers = get_headers()

    with open(image_path, "rb") as image_file:
        image_data = base64.b64encode(image_file.read()).decode("utf-8")

    data = {
        "faceset_id": faceset_id,
        "name": name,
        "project_id": project_id,
        "store_enrollment_images":True,
        "duplicate_face_confidence_threshold":0.5,
        "register_on_duplicate":False,
        "base64image_with_landmark": [
            {
                "base64image": image_data
            }
        ]
        
    }

    try:
        response = requests.post(endpoint, json=data, headers=headers)
        print("Response Content:", response.content)
        print("Response Status Code:", response.status_code)
    except requests.exceptions.RequestException as e:
        print(f"An error occurred: {e}")

#if __name__ == "__main__":
    #image_path = "C:/Users/hande/OneDrive/Desktop/API/venv/Lib/site-packages/Santosh.jpg"
    #create_face(image_path)

@app.route('/compare', methods=['POST'])
def compare():
    global temp_URL
    api_key = "ZI6JGNIWI3IARPNWIADJBMUVPJKAIJAPBYH9ZHW8Q0U7QX4I7QK2IFJIFTJATEO58K9ZX7F9OM8MNHPNFBSW07AFZ5SOT1GP7SZAPDSVBFDNGIJ2SGUQ99CRUQ5IBBVE"
    face_lookup_result = perform_face_lookup(lookup_request_data)
    print("Face Lookup Result:", json.dumps(face_lookup_result, indent=4))
    face_landmark_lookup_result = perform_face_landmark_lookup(lookup_request_data)
    print("Face Landmark Lookup Result:", json.dumps(face_landmark_lookup_result, indent=4))


    confidence_threshold = 0.6  

    if "lookup_results" in face_landmark_lookup_result:
        lookup_result = face_landmark_lookup_result["lookup_results"][0] 
        confidence_score = lookup_result.get("confidence_score", 0)  

        if confidence_score > confidence_threshold:
            print("Face recognized with confidence:", confidence_score)
            print("Face Token:", lookup_result["best_matched_face_token"])
            recognized_name = "Unknown"
            for lookup_result_face in face_lookup_result.get("lookup_results", []):
                if lookup_result_face.get("best_matched_face_token") == lookup_result["best_matched_face_token"]:
                    recognized = lookup_result_face.get("best_matched_face_token", "Unknown")
                    for face_info in faces_data:
                        if face_info.get("face_token") == lookup_result["best_matched_face_token"]:
                            recognized_name = face_info.get("name", "Unknown")
                            break

                break
                

            print("Recognized Name:", recognized_name)


        else:
            print("Unknown face. Confidence:", confidence_score)
    else:
        print("No face landmark lookup results.")

    if confidence_score > confidence_threshold:
        message1 = "Driver is authenticated."
        summary = "driver_verified"
        
    else:
        message1 = "Driver is not authenticated."
        summary = "driver_not_verified"
    if confidence_score == -1:
        message1 = "Driver is not prsent in vechical."
        summary = ""
    print(summary)
    response_data, requested_id = send_recording_request(api_key, length, content, endpoint_id,summary,start_time)
    print("Response Data:", response_data)
    print("Requested ID:", requested_id)
    return render_template('index1.html', message=message1)
if __name__ == '__main__':
    app.run()
    
    
    